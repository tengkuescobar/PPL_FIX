...
## NOTE ##
File berikut dipakai bersama. Jangan diedit sepihak tanpa koordinasi tim:
...
```
> Pastikan kolom: `title`, `description`, `syllabus` (JSON/text), `price`, `thumbnail`, `is_published`, `tutor_id`, `category`.  
> Module: `title`, `type` (video/text), `video_url`, `content`, `order`, `course_id`.
...
app/Http/Controllers/BookingController.php
    ...
    processPayment()→ proses bukti bayar
...
> Chat: `sender_id`, `receiver_id`, `message`, `is_read`.
...
app/Http/Controllers/ChatController.php
    index()   → daftar kontak chat (tutor yang pernah di-chat)
    show()    → percakapan dengan user/tutor tertentu
    send()    → kirim pesan (simpan ke DB)
...
resources/views/chat/index.blade.php     ← UI live chat (daftar kontak + area percakapan)
```
...
resources/views/marketplace/show.blade.php     ← detail produk + tombol add to cart
...
app/Http/Controllers/Admin/TransactionController.php
    index(), show(), updateStatus() → admin kelola transaksi
```
...
resources/views/orders/                          ← riwayat transaksi user
...
### Konvensi Branch
```
epic1/course-catalog
epic2/learning-system
epic3/tutor-homevisit
epic4/forum-chat
epic5/marketplace
epic6/transaction-payment
epic7/user-profile
```
